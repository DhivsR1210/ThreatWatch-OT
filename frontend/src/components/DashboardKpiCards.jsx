  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {cards.map(({ label, value, icon: Icon, iconClass }) => (
        <article key={label} className="glass-panel rounded-3xl border p-5 transition hover:-translate-y-1 hover:border-cyan-400/20">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm font-medium uppercase tracking-[0.24em] text-slate-500">{label}</p>
            <span className={`glass-subpanel grid h-11 w-11 place-items-center rounded-2xl ${iconClass}`}>
