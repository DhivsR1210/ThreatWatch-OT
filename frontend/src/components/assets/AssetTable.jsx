        ...
        <tbody className="divide-y divide-slate-800/80">
          {assets.map((asset) => (
            <tr className="transition hover:bg-slate-800/30" key={asset.id}>
              ...
              <td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${criticalityStyles[asset.criticality]}`}>{asset.criticality}</span><p className="mt-2 inline-flex items-center gap-1.5 text-xs text-slate-400"><span className={`size-1.5 rounded-full ${statusStyles[asset.operational_status]}`} />{asset.operational_status}</p></td>
